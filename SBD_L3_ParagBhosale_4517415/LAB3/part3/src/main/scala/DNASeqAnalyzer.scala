...
//===============================Get required directories=================================
	...
	val fastqChunk= config.getTmpFolder + x
	val outFileName = config.getTmpFolder + x + ".sam"
	val outputFolder = config.getOutputFolder
//===============================Set and download files ========================================
	val logfile= StringUtils.substringBefore(fastqChunk,".fq.gz")+"_log.txt"
	//log file for bwa 
	...
	//input chunk to bwa after unzipping hence using substringBefore to get name chunk1.fq
	
	val downloadfile= new Filemove(tmpFolder,inputFolder+x)
	downloadfile.download()
	//download zipped chunk files from HDFS  
	Seq("gunzip",tmpFolder+x)!
	...
	// bwa mem refFolder/RefFileName -p -t numOfThreads fastqChunk > outFileName
	//println(toolsFolder+"bwa "," mem "+ refFolder+RefFileName, " -p  -t "+ numOfThreads+ fastqChunk+" "+ outFileName)
...
//=========================bwa mem algorithm===================================================
	...
	//logging
	Seq(toolsFolder+"bwa","mem", refFolder+RefFileName, "-p", "-t" ,numOfThreads, chunkin) #> new File(outFileName)!
	//this will create unix command and execute=>  bwa mem refFolder/RefFileName -p -t numOfThreads fastqChunk > outFileName	
...
//=========================Getting key value pairs===========================================
	...
	Files.deleteIfExists(Paths.get(chunkin))
	//Delete temp input chunk file
	
	logbwa.write(getTimeStamp()+": KV value pairs:"+kvPairs.length)
	logbwa.close()
	val uploadlog= new Filemove(logfile,outputFolder+"log/bwa")
	uploadlog.upload()
	//upload the log to HDFS 
	Files.deleteIfExists(Paths.get(logfile))
	//delete log from temp directory
	//val uploadfile= new Filemove(tmpFolder + "result.vcf",outputFolder)
	//downloadfile.upload()
	return kvPairs;
...
{	
	...
	val logfile= tmpFolder+"chunk"+chrRegion+"_vari_log.txt"
	...
	//delete temp files 
	logvari.close()
	//====================================upload logs to the HDFS=====================
	val uploadlog= new Filemove(logfile,outputFolder+"log/vc")
	uploadlog.upload()
	//=================================================================================
	Files.deleteIfExists(Paths.get(logfile))
...
def main(args: Array[String]) 
{
	...
	//======================reading input chunks from HDFS ==========================================================
	val FSPath = new Path(inputFolder)
	//get input chunk path
	val rawfiles = FileSystem.get(sc.hadoopConfiguration).listStatus(FSPath).map( x => StringUtils.substringAfter((x.getPath).toString,inputFolder))
	//get list of files from HDFS input folder 
	//use listStatus to get information about FSPath in HDFS 
	//use .getPath to get path to .gz input chunk files 
	 
	println("Input files====================")
	rawfiles.foreach(println)
	println("===============================")

	val chunks = sc.parallelize(rawfiles,numInstances)
	//parallelize the input chunks according to numInstances

	//==============================================bwa call =======================================================
	val SAMRDD = chunks.mapPartitions{partitionIterator=>partitionIterator.map(a=>bwaRun(a,configBroad.value))}
	...
	// Sort the records by chr num 
	//further sort them by the position of the read.

		//============================= writing final output file===================================================
	val vcfLines = variRDDSort.flatMapValues(v=>v).map(x => x._2._2).collect
	// This RDD will have combined data of all vcf files
	val outputFolder = config.getOutputFolder
	val bw = new FileWriter(new File(tmpFolder + "final.vcf"))
	...
	bw.close()
	//==================================Uploading to HDFS ===========================================================
	val uploadfile= new Filemove(tmpFolder + "final.vcf",outputFolder)
	uploadfile.upload()
	//call upload method from external Filemove library
	
	Files.deleteIfExists(Paths.get(tmpFolder + "final.vcf"))
	//Delete temp final output file from local directory
