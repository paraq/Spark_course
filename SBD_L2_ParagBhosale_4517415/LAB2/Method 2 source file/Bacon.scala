...
//>>>>>>>>>
		//aggregateByKey will combine the co actors to create (actors,List<coactors>)
val redu_list=joined.aggregateByKey(ListBuffer.empty[String])((numList, num) => {numList += num; numList},(numList1, numList2) => {numList1.appendAll(numList2); numList1}).mapValues(_.toList)		
		...
		for (i <- 1 to 6)
 {
		...
		val iter_rdd = sc.parallelize(z)		//Load z array into iteration RDD (actor,distance):: this RDD will be updated at each iteration
		val finale_join=iter_rdd.join(redu_list)	//join data to get (actor,(distance,list<actors>)
		//finale_join.take(10).foreach(println)

		val redu_1 = finale_join.flatMap {case (a,(k, xs)) => for ((d) <- xs) yield ( d,k+1)}.reduceByKey(math.min(_ , _)).persist()
		redu_1.setName("iteration RDD "+i)
		//Now first flatten join output to get (actors,distance+1). Note actors in this RDD are from list from the values.
		//Next step is to get ruduceBykey to get minimum distance 
...
outfile.write("There are "+(count_1_M-previous_M)+" ("+(((count_1_M-previous_M).toFloat/total_M)*100)+") actors and "+(count_1_F-previous_F)+" ("+(((count_1_F-previous_F).toFloat/total_F)*100)+") actresses at distance "+i+"\n")	
		...
		if (i==6)
		{	
	val dist_6=iter_rdd.join(redu_1).filter{case(a,(b,c))=> b>c}.map{case(a,(b,c))=>(a,b)}.sortBy(_._1).map{case(a,b)=>(a)}
